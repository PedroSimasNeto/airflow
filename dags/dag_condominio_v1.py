...
from etl import Jobs_condominios
...
def st_condominios():
    ...
    parametro = Jobs_condominios(
        url=cfg["condominios"], header=cfg_secrets, database="postgres-datalake"
    )
    parametro.st_importar_condominios(table="st_condominio", schema="staging")

    data_fim = datetime.now() - data_inicio
    print(
        f"Task finalizado: {datetime.now().strftime('%Y-%m-%d')} \n Tempo total: {data_fim}"
    )
...
def st_relatorio_receitas_despesas(data_execucao):
    ...
    parametro = Jobs_condominios(
        url=cfg["relatorios"], header=cfg_secrets, database="postgres-datalake"
    )
    parametro.st_relatorio_receita_despesa(
    ...
    data_fim = datetime.now() - data_inicio
    print(
        f"Task finalizado: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')} \n Tempo total: {data_fim}"
    )
...
with DAG(
    dag_id="dag_administradora_condominio_v1",
    default_args=default_args,
    description="DAG Administradora Condomínio",
    schedule_interval="30 2 * * 0",
    tags=["condominios", "release_0.0.0"],
    catchup=False,
    max_active_runs=1,
        ...
        sql=[
            ...
            """
             INSERT INTO DIM_CONDOMINIO(id_condominio, id_planoconta, nome_condominio, fantasia_condominio, cep_condominio, cpf_cnpj_condominio,
                                        endereco_condominio, complemento_condominio, bairro_condominio, cidade_condominio, uf_condominio, fl_ativo)
                SELECT
    ...
    task_dimensao_conta_despesa = PostgresOperator(
        ...
        postgres_conn_id="postgres-datalake",
        sql=[
            "TRUNCATE TABLE DIM_CONTA_DESPESA;",
            """
    ...
    task_fato_relatorio_despesa = PostgresOperator(
        ...
        postgres_conn_id="postgres-datalake",
        sql=[
            f"DELETE FROM FATO_RECEITA_DESPESA WHERE DATA BETWEEN cast('{data_fato}' as date) - interval '{cfg['intervalo_execucao']} Month' and '{data_fato}';",
    ...
    (
        inicio
        >> st_condominios()
        >> task_dimensao_condominio
        >> st_relatorio_receitas_despesas("{{ next_ds }}")
        >> [task_dimensao_conta_despesa, task_fato_relatorio_despesa]
        >> fim
    )
